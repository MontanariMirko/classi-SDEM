...
public:
	...
	lzss(unsigned dim) : _dimMaxDiz(dim) {}
	...
	void codifica(fstream& f, string nome){
		...
		while(i < _buff.size()){
			...
			//se la lunghezza è inferiore di 3 scrivo solo i carattere
			if(tmp.lunghezza < 3){
				tmp.c = _buff[i];
				cout << "1 \t" << tmp.c << endl;
				fout.write((const char*)"1",1);
				fout.write((const char*)&tmp.c,1);
				++i;
			}
			else{
				cout << "0 \t" << tmp.posizione << "\t" << tmp.lunghezza << endl;
				fout.write((const char*)"0",1);
